...
CREATE TABLE `sto_unit` (
  ...
  PRIMARY KEY (`unit_id`),
  KEY `fk_unit_broker` (`brk_id`)
);
CREATE TABLE `sto_family` (
  `fam_id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `brk_id` bigint(20) unsigned NOT NULL,
  `fam_name` varchar(255) DEFAULT NULL,
  `fam_desc` longtext DEFAULT NULL,
  `fam_parent_id` bigint(20) unsigned DEFAULT NULL,
  `fam_position` int(11) DEFAULT NULL,
  `fam_left` int(11) DEFAULT NULL,
  `fam_right` int(11) DEFAULT NULL,
  `fam_level` int(11) DEFAULT NULL,
  `fam_code_int` enum('DRUG','OTHER') NOT NULL DEFAULT 'OTHER',
  PRIMARY KEY (`fam_id`),
  KEY `fk_family_family` (`fam_parent_id`),
  KEY `fk_family_broker` (`brk_id`),
  CONSTRAINT `fk_family_family` FOREIGN KEY (`fam_parent_id`) REFERENCES `sto_family` (`fam_id`) ON DELETE CASCADE
);
CREATE TABLE `sto_item` (
  ...
  `item_from` timestamp NULL DEFAULT NULL,
  `item_to` timestamp NULL DEFAULT NULL,
  `item_desc` longtext DEFAULT NULL,
  `item_stock` enum('STOCK','NONE') NOT NULL DEFAULT 'NONE',
  `item_type` enum('DRUG','OTHER') NOT NULL DEFAULT 'OTHER',
  `item_mark` varchar(80) DEFAULT NULL,
  `item_color` varchar(80) DEFAULT NULL,
  `item_weight` decimal(14,2) DEFAULT NULL,
  `item_width` decimal(14,2) DEFAULT NULL,
  `item_height` decimal(14,2) DEFAULT NULL,
  `item_depth` decimal(14,2) DEFAULT NULL,
  `item_dangerous` tinyint(1) NOT NULL DEFAULT 0,
  `item_qte` decimal(8,6) DEFAULT NULL,
  `item_min_qte` decimal(8,6) DEFAULT NULL,
  `item_disposition` text DEFAULT NULL,
  `item_sto_unit_id` bigint(20) unsigned DEFAULT NULL,
  `item_sto_qte` decimal(8,6) DEFAULT NULL,
  `item_content_unit_id` bigint(20) unsigned DEFAULT NULL,
  `item_content_qte` decimal(8,6) DEFAULT NULL,
  `fam_id` bigint(20) unsigned DEFAULT NULL,
  `item_order` int(11) DEFAULT NULL,
  `default_provider_id` bigint(20) unsigned DEFAULT NULL,
  PRIMARY KEY (`item_id`),
  KEY `fk_item_broker` (`brk_id`),
  KEY `fk_item_family` (`fam_id`),
  KEY `fk_item_stock_unit` (`item_sto_unit_id`),
  KEY `fk_item_content_unit` (`item_content_unit_id`),
  KEY `fk_item_default_provider` (`default_provider_id`),
  CONSTRAINT `fk_item_content_unit` FOREIGN KEY (`item_content_unit_id`) REFERENCES `sto_unit` (`unit_id`),
  CONSTRAINT `fk_item_default_provider` FOREIGN KEY (`default_provider_id`) REFERENCES `crm_client` (`cli_id`),
  CONSTRAINT `fk_item_family` FOREIGN KEY (`fam_id`) REFERENCES `sto_family` (`fam_id`),
  CONSTRAINT `fk_item_stock_unit` FOREIGN KEY (`item_sto_unit_id`) REFERENCES `sto_unit` (`unit_id`)
);